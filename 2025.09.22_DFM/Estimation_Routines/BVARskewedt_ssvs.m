function [A, SIGMA, A_samples, SIGMA_samples] = BVARskewedt_ssvs(Y, p, constant, num_samples, varargin)
%% ================================================================
% BVARskewedt_ssvs - Bayesian VAR with Skewed-t Innovations and SSVS Prior
% Implements multivariate skewed-t errors with SSVS variable selection
% Based on Karlsson, Mazur & Nguyen (JEDC 2023) + SSVS priors
%% ================================================================
%  INPUT
...
%                          'tau0', scalar (default: sqrt(0.1))
%                          'tau1', scalar (default: sqrt(4)) 
%                          'pi0', scalar (default: 0.1)
%% ================================================================
%  OUTPUT  
...
addParameter(parser, 'tau0', sqrt(0.1), @isnumeric);  % Small variance (spike)
addParameter(parser, 'tau1', sqrt(4), @isnumeric);    % Large variance (slab)
addParameter(parser, 'pi0', 0.1, @isnumeric);         % Prior inclusion probability
parse(parser, varargin{:});
verbose = parser.Results.verbose;
burnin = parser.Results.burnin;
thin = parser.Results.thin;
tau0 = parser.Results.tau0;
tau1 = parser.Results.tau1;
pi0 = parser.Results.pi0;

% Prepare data using existing function structure
[Y, X, M, T, KK, ~] = prepare_BVAR_matrices(Y, p, constant);

if verbose
    fprintf('Estimating Bayesian VAR with Skewed-t Errors and SSVS Prior\n');
    fprintf('Data: T=%d, M=%d, KK=%d\n', T, M, KK);
    fprintf('SSVS: tau0=%.3f, tau1=%.3f, pi0=%.3f\n', tau0, tau1, pi0);
...
GAMMA = ones(KK, M);        % SSVS indicators (start with all included)

% SSVS priors
% tau0, tau1, pi0 set from inputs

% gamma: normal N(0, 10) - keep same as before
gamma_prior_var = 10;

% nu: truncated gamma G(2, 0.1) on [4.1, 100] - keep same as before
nu_prior_a = 2;
nu_prior_b = 0.1;
nu_min = 4.1;
nu_max = 100;

% SIGMA: inverse Wishart IW(M+2, I) - keep same as before
Psi_0 = eye(M);
v_0 = M + 2;
...
for iter = 1:total_draws
    ...
    %% Step 1: Sample GAMMA (SSVS indicators)
    GAMMA = sample_SSVS_indicators(B, tau0, tau1, pi0, KK, M);
    
    %% Step 2: Sample B and gamma jointly (with SSVS prior)
    [B, gamma] = sample_B_gamma_ssvs(Y, X, xi, B, gamma, GAMMA, M, T, KK, tau0, tau1, gamma_prior_var, SIGMA);
    
    %% Step 3: Sample SIGMA 
    SIGMA = sample_SIGMA(Y, X, B, gamma, xi, Psi_0, v_0, M, T);
    
    %% Step 4: Sample nu (degrees of freedom)
    nu = sample_nu(xi, nu, nu_prior_a, nu_prior_b, nu_min, nu_max, M, T);
    
    %% Step 5: Sample xi (mixing variables)
...
function GAMMA = sample_SSVS_indicators(B, tau0, tau1, pi0, KK, M)
% Sample SSVS indicator variables
...
for i = 1:M
    for j = 1:KK
        ...
        l_0 = -0.5 * log(2*pi*tau0^2) - 0.5 * (B(j,i)/tau0)^2;  % spike
        l_1 = -0.5 * log(2*pi*tau1^2) - 0.5 * (B(j,i)/tau1)^2;  % slab
...
function [B, gamma] = sample_B_gamma_ssvs(Y, X, xi, B, gamma, GAMMA, M, T, KK, tau0, tau1, gamma_prior_var, SIGMA)
% Sample B and gamma jointly with SSVS prior structure

mu_xi = mean(xi, 1);  % 1 x M

% Transform data using mixing variables
Y_trans = zeros(T, M);
...
% SSVS Prior precision matrix
total_params = KK*M + M;
V_prior_inv = zeros(total_params, total_params);

% B coefficients - SSVS structure
for i = 1:M
    for j = 1:KK
        idx = (i-1)*KK + j;
        if GAMMA(j, i) == 1
            V_prior_inv(idx, idx) = 1 / tau1^2;  % slab (large variance)
        else
            V_prior_inv(idx, idx) = 1 / tau0^2;  % spike (small variance)
        end
    end
end

% gamma coefficients - same as before
for i = 1:M
    idx_gamma = KK*M + i;
...
% Sample using Cholesky with error handling
try
